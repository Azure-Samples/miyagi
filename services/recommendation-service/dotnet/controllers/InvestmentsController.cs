...
{
    private string _memoryCollection = Env.Var("MEMORY_COLLECTION");
    ...
    public async Task<IActionResult> GetRecommendations([FromBody] MiyagiContext miyagiContext)
    {
        ...
        var searchResults = _kernel.Memory.SearchAsync(_memoryCollection, "investment advise", limit: 3, minRelevanceScore: 0.8);

        await foreach (var item in searchResults)
        {
            log?.LogDebug(item.Metadata.Text + " : " + item.Relevance);
        }
